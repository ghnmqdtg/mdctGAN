...
class Audio2MDCT(torch.nn.Module):
    ...
    def to_spectro(self, audio: torch.Tensor, mask: bool = False, mask_size: int = -1):
        ...
        if mask:
            ...
            log_spectro = torch.cat((log_spectro[:, :, :, :-mask_size], _noise), dim=3)
        return (
            log_spectro.float(),
            pha,
            {
    ...
    def normalize(self, spectro):
        if self.explicit_encoding:
            ...
            log_spectro = torch.cat(
                (
                    aF.amplitude_to_DB(
                        self.alpha * pos + (1 - self.alpha) * neg,
                        20.0,
                        self.min_value,
                        1.0,
                    ),
                    aF.amplitude_to_DB(
                        (1 - self.alpha) * pos + self.alpha * neg,
                        20.0,
                        self.min_value,
                        1.0,
                    ),
            ...
            log_spectro = torch.arcsinh(spectro) / torch.log(torch.tensor(10.0))
        ...
        if not self.abs_norm:
            audio_max = (
                log_spectro.flatten(-2).max(dim=-1).values[:, :, None, None].float()
            )
            audio_min = (
                log_spectro.flatten(-2).min(dim=-1).values[:, :, None, None].float()
            )
        else:
        ...
        log_spectro = (
            log_spectro * (self.norm_range[1] - self.norm_range[0]) + self.norm_range[0]
        )

        return log_spectro, audio_max, audio_min, mean, std

    def denormalize(
        self, log_spectro: torch.Tensor, min: torch.Tensor, max: torch.Tensor
    ):
        log_spectro = (log_spectro.to(torch.float64) - self.norm_range[0]) / (
        ...
        if self.arcsinh_transform:
            return (
                torch.sinh(log_spectro * torch.log(torch.tensor(10.0)))
                / self.arcsinh_gain
            )
        if self.raw_mdct:
            return log_spectro
    ...
    def to_audio(
        self,
        log_spectro: torch.Tensor,
        norm_param: Dict[str, torch.Tensor],
        pha: torch.Tensor,
    ):
        ...
        if self.explicit_encoding:
            ...
            if self.up_ratio > 1:
                ...
                psudo_pha = (
                    2
                    * torch.randint(low=0, high=2, size=pha.size(), device=self.device)
                    - 1
                )
                pha = torch.cat(
                    (
                        pha[..., : int(size * (1 / self.up_ratio)), :],
                        psudo_pha[..., int(size * (1 / self.up_ratio)) :, :],
                    ),
                    dim=-2,
                )
                spectro = spectro * pha
    ...
    def norm_frames(self, frames):
        frames = aF.amplitude_to_DB(torch.abs(frames), 20, self.min_value, 1)
    ...
    def hr_forward(self, hr_audio: torch.Tensor):
        ...
        with torch.no_grad():
            hr_spectro, hr_pha, hr_norm_param = self.to_spectro(
                hr_audio,
                mask=self.mask_hr,
                mask_size=int(
                    self.n_fft
                    * (1 - self.sr_sampling_rate / self.hr_sampling_rate)
                    // 2
                ),
            )
...
class Pix2PixHDModel(BaseModel):
    ...
    def initialize(self, opt):
        ...
        # set freeze network
        self.freeze = (
            opt.freeze_g_d or opt.freeze_g_u or opt.freeze_l_d or opt.freeze_l_u
        )
        ...
        self.netG = networks.define_G(
            ...
            input_size=(opt.bins, opt.n_fft // 2),
            n_attn_g=opt.n_blocks_attn_g,
            n_attn_l=opt.n_blocks_attn_l,
            proj_factor_g=opt.proj_factor_g,
            heads_g=opt.heads_g,
            dim_head_g=opt.dim_head_g,
            proj_factor_l=opt.proj_factor_l,
            heads_l=opt.heads_l,
            dim_head_l=opt.dim_head_l,
        )
        self.netG.set_freeze(
            opt.freeze_g_d, opt.freeze_g_u, opt.freeze_l_d, opt.freeze_l_u
        )
        ...
        if self.isTrain:
            ...
            print(
                "Total number of parameters of G: %d"
            ...
            print(
                "Total number of parameters of D: %d"
                % (sum([param.numel() for param in params]))
            )
    ...
    def _forward(self, lr_audio, hr_audio, infer=False):
        (
            ...
            _,
            lr_norm_param,
        ) = self.forward(lr_audio, hr_audio)
        ...
        pred_fake_pool = self.discriminate_F(lr_spectro, sr_input, use_pool=True)
        ...
        pred_fake = self.netD.forward(torch.cat((lr_spectro, sr_input), dim=1))
        ...
        if self.arcsinh_transform:
            self.current_lable = (lr_spectro.detach().cpu() - self.norm_range[0]) / (
                self.norm_range[1] - self.norm_range[0]
            )
            self.current_lable = (
                self.current_lable
                * (lr_norm_param["max"].cpu() - lr_norm_param["min"].cpu())
                + lr_norm_param["min"].cpu()
            ).numpy()[0, 0, :, :]

            min_val = hr_norm_param["min"].cpu().numpy()[0, 0, :, :]
            max_val = hr_norm_param["max"].cpu().numpy()[0, 0, :, :]
            self.current_generated = (
                sr_spectro.detach().cpu() - self.norm_range[0]
            ) / (self.norm_range[1] - self.norm_range[0])
            self.current_generated = np.clip(
                (
                    self.current_generated
                    * (lr_norm_param["max"].cpu() - lr_norm_param["min"].cpu())
                    + lr_norm_param["min"].cpu()
            ...
            self.current_real = (hr_spectro.detach().cpu() - self.norm_range[0]) / (
                self.norm_range[1] - self.norm_range[0]
            )
            self.current_real = (
                self.current_real
                * (hr_norm_param["max"].cpu() - hr_norm_param["min"].cpu())
                + hr_norm_param["min"].cpu()
            ).numpy()[0, 0, :, :]
        else:
            self.current_lable = lr_spectro.detach().cpu().numpy()[0, 0, :, :]
            self.current_generated = sr_spectro.detach().cpu().numpy()[0, 0, :, :]
        ...
        return [
            ...
            torch.empty if not infer else sr_spectro,
        ]
    ...
    def flops(self, lr_audio):
        ...
        Gflops, unsupported = flop_count(
            model=model, inputs=(lr_audio), supported_ops=supported_ops
        )
        statics = summary(model, input_data=[lr_audio], verbose=0)
        del model
