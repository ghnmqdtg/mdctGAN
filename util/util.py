...
def imdct(
    ...
    spectro = torch.abs(spectro) * (
        norm_param["max"].to(device) - norm_param["min"].to(device)
    ) + norm_param["min"].to(device)
    ...
    if explicit_encoding:
        ...
        if up_ratio > 1:
            ...
            pha = torch.cat(
                (
                    pha[..., : int(size * (1 / up_ratio)), :],
                    psudo_pha[..., int(size * (1 / up_ratio)) :, :],
                ),
                dim=-2,
            )
    else:
        if up_ratio > 1:
            size = pha.size(-2)
            psudo_pha = (
                2 * torch.randint(low=0, high=2, size=pha.size(), device=device) - 1
            )
            pha = torch.cat(
                (
                    pha[..., : int(size * (1 / up_ratio)), :],
...
def kbdwin(N: int, beta: float = 12.0, device="cpu") -> torch.Tensor:
    ...
    w = torch.kaiser_window(
        window_length=N // 2 + 1, beta=beta * torch.pi, periodic=False, device=device
    )
    w_sum = w.sum()
