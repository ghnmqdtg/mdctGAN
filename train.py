...
for epoch in range(start_epoch, opt.niter + opt.niter_decay + 1):
    ...
    for i, data in enumerate(train_dataloader, start=epoch_iter):
        if end:
            print(
                "exiting and saving the model at the epoch %d, iters %d"
                % (epoch, total_steps)
            )
            model.save("latest")
            model.save(epoch)
        ...
        loss_D = (
            (loss_dict["D_fake"] + loss_dict["D_real"]) * 0.5
            + (loss_dict.get("D_fake_t", 0) + loss_dict.get("D_real_t", 0)) * 0.5
            + (loss_dict.get("D_fake_mr", 0) + loss_dict.get("D_real_mr", 0)) * 0.5
        )
        loss_G = (
            loss_dict["G_GAN"]
            + loss_dict.get("G_mat", 0)
            + loss_dict.get("G_GAN_Feat", 0)
            + loss_dict.get("G_VGG", 0)
            + loss_dict.get("G_GAN_t", 0)
            + loss_dict.get("G_GAN_mr", 0)
            + loss_dict.get("G_shift", 0)
        )
