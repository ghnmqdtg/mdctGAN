...
audio_len = data_loader.train_dataset.raw_audio.size(-1)
_mse, _snr_sr, _snr_lr, _ssnr_sr, _ssnr_lr, _pesq, _lsd, _lsd_hf, _lsd_lf = (
    compute_matrics(
        data_loader.train_dataset.raw_audio,
        data_loader.train_dataset.lr_audio[..., :audio_len],
        audio[..., :audio_len],
        opt,
    )
)
...
# print('PESQ: %.4f' % _pesq)
print("LSD: %.4f" % _lsd)
print("LSD_HF: %.4f" % _lsd_hf)
print("LSD_LF: %.4f" % _lsd_lf)
...
torchaudio.save(lr_path, data_loader.train_dataset.lr_audio.cpu(), opt.hr_sampling_rate)
torchaudio.save(
    hr_path,
    data_loader.train_dataset.raw_audio.cpu(),
